...
uses CarteiraDM, FerramentasForm, NovoCarteiraForm;

procedure TCadastroCarteiraFrm.actAlterarExecute(Sender: TObject);
begin
  if (grdCadastro.SelectedField.DisplayText<>'') then
  begin
    Application.CreateForm(TNovoCarteiraFrm,NovoCarteiraFrm);
    try
      NovoCarteiraFrm.OpCadastro:='A';
      NovoCarteiraFrm.Iniciar();
      NovoCarteiraFrm.ShowModal;
    finally
      FreeAndNil(NovoCarteiraFrm);
    end;
  end
  else
    FerramentasFrm.MsgPadrao('ESCOLHA UM REGISTRO','ALTERAR',1)
end;
...
procedure TCadastroCarteiraFrm.actNovoExecute(Sender: TObject);
begin
  Application.CreateForm(TNovoCarteiraFrm,NovoCarteiraFrm);
  try
    NovoCarteiraFrm.OpCadastro:='N';
    NovoCarteiraFrm.Iniciar();
    NovoCarteiraFrm.ShowModal;
  finally
    FreeAndNil(NovoCarteiraFrm);
